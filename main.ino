...
/**
 * @class HeartBeat
 * @brief Simple periodic toggler used for status LED/buzzer heartbeat.
 * @details
 * Produces periodic pulses based on a configured interval. Designed to be
 * non-blocking when used with update()/tick() calls from the main loop.
 */
class HeartBeat
{
    public:
...
void MachineState::begin()
{
    ...
    Serial.begin(9600);
    Serial.println("MachineState Initialized, port 9600");
...
unsigned long HeartBeat::getInterval() const
{
    return interval;
}

bool HeartBeat::getState() const
{
    return isOn;
}